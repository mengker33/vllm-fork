...
from typing import Any, Literal, Optional, Set, Tuple, TypedDict, TypeVar, Union
...
class InternVLDummyInputsBuilder(BaseInternVLDummyInputsBuilder[_I]):
...
class InternVLMultiModalProcessor(BaseInternVLMultiModalProcessor[_I]):
    ...
    def _get_mm_fields_config(
        ...
        if self.info.supports_video:
            ...
            video_fields = dict(
                pixel_values_video=MultiModalFieldConfig.flat_from_sizes(
...
class InternVLChatModel(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def extract_feature(self, image_input: torch.Tensor) -> torch.Tensor:
        ...
        for vision_bucket in sorted(self.vision_buckets.multimodal_buckets, reverse=True):
            ...
            while input_block_num >= vision_bucket:
                t += vision_bucket
                vit_embeds = self.vision_model(pixel_values=pixel_values[s:t].clone())  # (num_img_blocks, img_tokens+cls=1025, hiddensize=1024)
                s=t
    ...
    def _parse_and_validate_video_input(
        ...
        pixel_values_video = kwargs.pop("pixel_values_video", None)
        video_num_patches = kwargs.pop("video_num_patches", None)
        video_embeds = kwargs.pop("image_embeds", None)

        if pixel_values_video is None and video_embeds is None:
            return None
        ...
        if pixel_values_video is not None:
            if not isinstance(pixel_values_video, (torch.Tensor, list)):
                raise ValueError("Incorrect type of pixel values. "
                                 f"Got type: {type(pixel_values_video)}")

            if not isinstance(video_num_patches, (torch.Tensor, list)):
                raise ValueError("Incorrect type of image_num_patches. "
                                 f"Got type: {type(video_num_patches)}")

            pixel_values_video = flatten_bn(pixel_values_video,
                                                 concat=True)
        ...
        for input_key in kwargs:
            if input_key in ("pixel_values",
                ...
                modalities["images"] = self._parse_and_validate_image_input(
                    **kwargs)
            if input_key in ("pixel_values_video",
